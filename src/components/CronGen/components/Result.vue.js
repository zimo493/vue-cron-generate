...
export default await (async () => {
    ...
    watch(() => value.value, () => expressionChange());
    ...
    const expressionChange = () => {
        ...
        if (resultArr.length == 0) {
            ...
            if (resultArr.length !== 5) {
                resultList.value.push("最近100年内只有左边" + resultArr.length + "条结果！");
            }
        }
        // 计算完成-显示结果
        isShow.value = true;
    ...
    function __VLS_template() {
        ...
        __VLS_elementAsFunction(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({ ...{ class: ("popup-result") }, });
        if (__VLS_ctx.isShow) {
            for (const [item] of __VLS_getVForSourceType((__VLS_ctx.resultList))) {
        ...
        if (typeof __VLS_styleScopedClasses === 'object' && !Array.isArray(__VLS_styleScopedClasses)) {
            __VLS_styleScopedClasses['popup-result'];
        }
        var __VLS_slots;
        return __VLS_slots;
        const __VLS_componentsOption = {};
        const __VLS_name = "CronGenResult";
